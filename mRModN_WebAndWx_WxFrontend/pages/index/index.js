...
Page({
  ...
  onLoginTap() {
    ...
    wx.getUserProfile({desc:'用于保存分析任务与用户信息',success:(profile)=>{
      wx.login({success:(login)=>{if(!login.code)return this.finishLoginError('获取微信登录凭证失败');
        requestLogin({method:'POST',data:{loginCode:login.code,nickname:profile.userInfo.nickName,avatarUrl:profile.userInfo.avatarUrl}}).then((res)=>{
          if(!res.data||res.data.code!==0||!res.data.token)throw {message:(res.data&&res.data.error)||'登录失败'};
          ...
          wx.setStorageSync('userInfo',userInfo);wx.setStorageSync('sessionToken',res.data.token);this.setData({isLoggedIn:true,isLogging:false,userInfo});wx.showToast({title:'登录成功',icon:'success'});
        }).catch(err=>this.finishLoginError(err.message));},fail:()=>this.finishLoginError('微信登录失败')});
    },fail:()=>this.finishLoginError('需要授权后才能关联任务')});
  },
  finishLoginError(message){this.setData({isLogging:false});wx.showToast({title:message||'登录失败',icon:'none'});},
  onLogoutTap(){wx.showModal({title:'退出登录',content:'本机保存的会话将被清除，是否继续？',success:(res)=>{if(res.confirm){wx.removeStorageSync('userInfo');wx.removeStorageSync('sessionToken');this.checkLoginStatus();}}});},
  ...
  onSubmitTap(){
    ...
    if(!sequences.length){wx.showToast({title:'请至少输入一条序列',icon:'none'});return;}
    for(let i=0;i<sequences.length;i++){const check=validateSequence(sequences[i]);if(!check.valid){wx.showToast({title:`序列 ${i+1}：${check.message}`,icon:'none',duration:2500});return;}}
    ...
    requestWithFallback(ENDPOINTS.WX_SUBMIT_TASK,{method:'POST',data,timeout:30000}).then((res)=>{
      const id=res.data&&res.data.data&&res.data.data.job_id;if(!id)throw {message:'后端未返回任务 ID'};
    ...
    }).catch(err=>{this.setData({isSubmitting:false});if(err.statusCode===401)this.onSessionExpired();else wx.showToast({title:err.message||'提交失败',icon:'none'});});
  },
  onSessionExpired(){wx.removeStorageSync('sessionToken');this.checkLoginStatus();wx.showModal({title:'登录已失效',content:'请重新登录后继续。',showCancel:false});},
